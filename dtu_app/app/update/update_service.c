    ...
    switch(dev)
    {
        case DEV_SYS:
            ...
            if(!info->checksum)
            {
                ...
                LOG(INFO,"CheckSum %x",info->checksum);
            }
...
GM_ERRCODE update_service_create(bool first_create)
{
    ...
    if(!s_update_socket.fifo.baseAddr)
    {
    ...
    if(GM_SUCCESS != util_convertipaddr(addr, IP))
    {
        ...
        {
            gm_socket_set_addr(&s_update_socket, addr, strlen((const char *)addr), port, config_service_update_socket_type());
        }
        else
        {
