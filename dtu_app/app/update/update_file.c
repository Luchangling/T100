...
GM_ERRCODE update_filemod_create(void)
{
    ...
    if(GM_SUCCESS != util_convertipaddr(addr, IP))
    {
        ...
        {
            gm_socket_set_addr(&s_file_socket, addr, strlen((const char *)addr), port, config_service_update_socket_type());
        }
        else
        {
...
{
    if(!s_file_socket.fifo.baseAddr)
    {
        return GM_SUCCESS;
    }
...
void update_msg_pack_head(u8 *pdata, u16 *idx, u16 len)
{
    ...
    imei[15] = 0;
    #endif
    //memcpy(imei,"862459042850995",strlen("862459042850995"));

    LOG(INFO,"update_msg_pack_head imei %s",imei);
...
void update_msg_receive(SocketType *socket)
{
    ...
    */
    ...
    if(SUCCESS != fifo_peek(&socket->fifo, head, len))
    {
        // no msg
        return;
    }
    ...
    switch(req_result)
    {
        case UPDATE_NONE:
            if(update_file_device_id() == DEV_EXT)
            {
                ...
                LOG(INFO,"clock(%d) update_msg_parse_response %s no need to update, try %dh later again.",util_clock(),update_file_get_device_type() ,UPDATE_PING_TIME/3600);
            }
            else
            {
                ...
                LOG(INFO,"clock(%d) update_msg_parse_response %s no need to update, try %ds later again.",util_clock(),update_file_get_device_type() ,10);
            }
            
            break;
    
        case UPDATE_QUE:
            LOG(DEBUG,"clock(%d) update_msg_parse_response %s queued to update, try %ds later again.", util_clock(),update_file_get_device_type(), UPDATE_RETRY_TIME);
        ...
        case UPDATE_READY:
            ...
            LOG(INFO,"clock(%d) update_msg_parse_response file(%s) check_sum:0x%04X, len:%u.",util_clock(),update_file_get_device_type(), s_file_extend.check_sum,s_file_extend.total_len);
            ...
            if (strcmp((const char *)s_file_extend.custom_code, (const char *)CFG_CUSTOM_CODE) !=0)
            {
                LOG(INFO,"clock(%d) update_msg_parse_response compare custom_code(%s!=%s).",
                    util_clock(), s_file_extend.custom_code, CFG_CUSTOM_CODE);
                break;
            }
...
void update_msg_start_data_block_request(SocketType *socket)
{
    ...
    {
        LOG(ERR,"clock(%d) update_msg_start_data_block_request assert(total_len(%d)) failed.", util_clock(),s_file_extend.total_len);
        return;
    }
    ...
    if(s_file_extend.handle >= 0)
    {
        ...
        LOG(INFO,"clock(%d) update_filemod_file_create need close",util_clock());
...
static void update_msg_parse_check_block_bits(u16 block)
{
    ...
    if(s_file_extend.block_current >= s_file_extend.total_blocks)
    {
        ...
        else
        {
            ...
            LOG(INFO,"clock(%d) update_msg_parse_check_block_bits (%s)update_filemod_file_finish success.",update_file_get_device_type(),util_clock());
            update_filemod_destroy();
        }
...
static GM_ERRCODE update_filemod_file_finish(void)
{
    ...
    if(update_file_device_id() == DEV_SYS)
    {
        ...
        {
            LOG(INFO,"clcok(%d) Update system info len %x checksum %x flag %x crc %x",clock(),\
            s_upd_sys_info.file_len,s_upd_sys_info.checksum,s_upd_sys_info.update_flag,s_upd_sys_info.crc);
        }
        else
        {
            LOG(FATAL,"clock(%d) Update system write fail!",clock());
        }
    }
...
static GM_ERRCODE update_filemod_file_write(u32 fs_seek, u8* pdata, u32 len)
{
    ...
    if (fret < 0)
    {
        LOG(INFO,"clock(%d) update_filemod_file_write FS_Seek failed, ret:%d.", util_clock(),fret);
        FS_Close(s_file_extend.handle);
        s_file_extend.handle = -1;
        return GM_SYSTEM_ERROR;
    }
    ...
    {
        LOG(INFO,"clock(%d) update_filemod_file_write FS_Write failed. ret:%d, writen:%d.", util_clock(), fret, writen);
        FS_Close(s_file_extend.handle);
        s_file_extend.handle = -1;
        return GM_SYSTEM_ERROR;
    }
...
static GM_ERRCODE update_filemod_file_create(u32 fs_len, int *sys_error)
{
    ...
    *sys_error = 0;
    ...
    if (s_file_extend.handle < 0)
    {
        LOG(DEBUG,"clock(%d) update_filemod_file_create assert(FS_Open) failed.", util_clock());
...
u32 update_filemod_get_checksum(const u8 *FileName)
{
    ...
    if (handle < 0)
    {
        LOG(DEBUG,"clock(%d) update_filemod_get_checksum assert(FS_Open) failed. ret:%d", util_clock(), handle);
        FS_Close(handle);
        return 0;
    }
    ...
    if(buff == NULL)
    {
        LOG(WARN,"clock(%d) update_filemod_get_checksum(malloc error)",util_clock());
        FS_Close(handle);
        return 0;
    }
