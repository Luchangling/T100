...
GM_ERRCODE gm_socket_init(SocketType *socket, SocketIndexEnum access_id)
{
    ...
    {
        if(!fifo_Init(&(socket->fifo), 2*SIZE_OF_SOCK_FIFO))
        {
            LOG(FATAL,"clock(%d) scoket(id %d) fifo creat fail!",util_clock(),access_id);
        }
    ...
    else
    {
        ...
        if(!fifo_Init(&(socket->fifo), SIZE_OF_SOCK_FIFO))
        {
            LOG(FATAL,"clock(%d) scoket(id %d) fifo creat fail!",util_clock(),access_id);
        }
    }
...
void socket_get_host_by_name_callback(void *msg)
{
    ...
    {
        LOG(ERR,"clock(%d) socket_get_host_by_name_callback assert(current_get_host.socket != NULL) failed.", 
			util_clock());
        current_get_host.status = CURRENT_GET_HOST_INIT;
        return;
    }
...
GM_ERRCODE gm_socket_connect(SocketType *socket)
{
    ...
    if (socket->id >= 0)
    {
        ...
        if (SocketConnect(socket->id, ip_addr, socket->port, s_account_id, socket->access_id, socket->type))
        {
            ...
            {
                LOG(ERR,"clock(%d) gm_socket_connect unknown type(%d)(%d.%d.%d.%d:%d).", 
                    util_clock(), socket->type, socket->ip[0], socket->ip[1], socket->ip[2], socket->ip[3], socket->port);
				return GM_UNKNOWN;
            }
        }
...
{  
    ...
    {
        LOG(ERR,"clock(%d) gm_socket_send status(%d) error. type(%d)(%d.%d.%d.%d:%d) id(%d) len(%d).", 
    ...
    {
        LOG(ERR,"clock(%d) gm_socket_send type(%d)(%d.%d.%d.%d:%d) id(%d) len(%d) too long.", 
...
GM_ERRCODE gm_socket_recv(SocketType *socket)
{
	...
	if(socket->status <= SOCKET_STATUS_CONNECTING || socket->status >= SOCKET_STATUS_DATA_FINISH)
    {
        LOG(ERR,"clock(%d) gm_socket_recv status(%d) error. type(%d)(%d.%d.%d.%d:%d) id(%d).", 
            util_clock(), socket->status, socket->type, socket->ip[0], socket->ip[1], socket->ip[2], socket->ip[3], 
    ...
    if (NULL == recv_buff)
    {
        LOG(ERR,"clock(%d) gm_socket_recv type(%d)(%d.%d.%d.%d:%d) id(%d) memory not enough.", 
            util_clock(), socket->type, socket->ip[0], socket->ip[1], socket->ip[2], socket->ip[3], 
            socket->port,socket->id);
        return GM_MEM_NOT_ENOUGH;
    }
    ...
    do
    {
        ...
        if(recvlen > 0)
        {
            ...
            cl_log_hex(recv_buff,recvlen,"[DATA]clock(%d) socket(%d)<-",clock(),socket->id);
        }
        ...
        if (recvlen > 0)
        {
           // gm_socket_get_ackseq(socket, &socket->ack_seq);
    		...
    		if(ret != recvlen)
    		{
    			LOG(ERR,"clock(%d) fifo insert failed, return(%d)", util_clock(), ret );
            ...
            {
                LOG(INFO,"clock(%d) cur socket fifo len %d",util_clock(),fifo_GetLen(&socket->fifo));
            }
        }
    }while(recvlen > 0);
...
void gm_socket_get_host_timer_proc(void)
{
    ...
    {
        s_get_host_list[i].requested = false;
        LOG(ERR,"clock(%d) gm_socket_get_host_timer_proc assert(socket_idx(%d)) failed.", util_clock(), i );
        return;
    }
