...
GM_ERRCODE gps_data_insert_one(LocationCurData *pdata, HisDatFileType *phis, u8 *data, u8 len, bool log)
{
    ...
    {
        LOG(WARN,"gps_data_init(%p,%p) assert(MemoryAlloc(%d) failed",pdata, phis, len);
...
void gps_his_file_delete(HisDatFileType *phis)
{
    ...
    ret = FS_CheckFile(HIS_FILE_NAME);
    if (ret >= 0)
    {
        FS_Delete(HIS_FILE_NAME);
...
static u32 gps_his_file_write(HisDatFileType *phis, void * DataPtr, u32 Length, int Offset)
{
    ...
    if(phis->handle < 0)
    {
        phis->handle = FS_Open(HIS_FILE_NAME, 0);
        if (phis->handle < 0)
        {
...
static GM_ERRCODE gps_data_save_file(LocationCurData *pdata, HisDatFileType *phis)
{
    ...
    while(GM_SUCCESS == gps_data_peek_one(pdata, phis, &one, true))
    {
        ...
        {
            LOG(ERR,"gps_data_save_file assert(len(%d)) failed.", one.len);
            break;
        }
...
static void gps_his_file_delete_record(HisDatFileType *phis, u16 del_num)
{
    ...
    {
        LOG(ERR,"clock(%d) gps_his_file_delete_record. assert(record_num(%d)) failed.", util_clock(),phis->record_num);
        return;
    }
    if(phis->handle < 0)
    {
        LOG(ERR,"clock(%d) gps_his_file_delete_record. assert(handle(%d)) failed.", util_clock(),phis->handle);
        return;
    }
...
u16 gps_service_get_pending_cahce(void)
{
    return gps_data_get_len(&s_gps_sending);
}


void gps_service_confirm_cache(void)
{
    LocationSaveData one;
    ...
    if(GM_SUCCESS == gps_data_peek_one(&s_gps_sending, NULL, &one, true))
    {
