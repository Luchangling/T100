...
static SocketTypeExtend s_gps_socket_extend = {false,0,0,0,0,0,0,0,0};
...
{
    u8 IP[4];
    LOG(INFO,"clock(%d) gps_service_init_proc",util_clock());
...
static void gps_service_login_proc(void)
{
    ...
    if((current_time - s_gps_socket.send_time) >= MESSAGE_TIME_OUT)
    {
        ...
        {
            if(GM_SUCCESS != protocol_send_logout_msg(&s_gps_socket))
            {
                return;
            }
...
static void gps_service_work_proc(void)
{
    ...
    //bool lbs_enable;
    u32 send_counts = 0;
    static u32 send_bms_time = 10;


    if(PROTOCOL_JT808 == config_service_get_app_protocol() && VEHICLE_STATE_STATIC == system_state_get_vehicle_state() )
    {
        ...
        //protocol_send_transiver_data_msg(&s_gps_socket);
    }

    if(bms_service_state())
    {
        if(current_time - send_bms_time > 30)
        {
            protocol_send_bms_data_msg(&s_gps_socket);
            
            send_bms_time = current_time;
        }
...
GM_ERRCODE gps_service_create(bool first_create)
{
    ...
    {
        #if 0
        idx = sscanf((const char *)config_service_get_pointer(CFG_TEST_SERVERADDR), "%[^:]:%d", addr, &port);
        if (idx != 2)
        {
    ...
    if(GM_SUCCESS != ConvertIpAddr(addr, IP))
    {
        if(util_is_valid_dns(addr, strlen((const char *)addr)))
        {
            gm_socket_set_addr(&s_gps_socket, addr, strlen((const char *)addr), port, STREAM_TYPE_STREAM);
			system_state_get_ip_cache(SOCKET_INDEX_MAIN, IP);
...
{    
    gps_service_confirm_cache();

    s_gps_socket.confirm_time = util_clock();
}
...
GM_ERRCODE gps_service_push_gps(GpsDataModeEnum mode, const GPSData *gps)
{
    ...
    if(SOCKET_STATUS_WORK == s_gps_socket.status)
    {
        ...
        #if 0
        if(s_gps_socket.send_time - s_gps_socket.confirm_time  < 30)
        {
            
        }
        else
        {
            LOG(ERR,"clock(%d) upto 30s no ack gps_service_destory last ack time(%d)!",s_gps_socket.send_time,util_clock());
            gps_service_destroy();
        }
        #else
...
GM_ERRCODE gps_service_push_alarm(AlarmInfo *alarm)
{
    ...
    {
    	LOG(ERR,"Failed to protocol_pack_alarm_msg!");
        return GM_SYSTEM_ERROR;
    }
...
GM_ERRCODE gps_service_push_position_request(u8 *mobile_num, u8 num_len, u8 *command, u8 cmd_len)
{
    ...
    {
        LOG(ERR,"Failed to gps_service_push_position_request!");
        return GM_SYSTEM_ERROR;
    }
...
void gps_service_recv_location_ack(void)
{
    gps_service_confirm_gps_cache();
}
