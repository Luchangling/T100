...
{
    TRAN_GNSS_DATA = 0,
    TRAN_GNSS_IC_INFO = 0x0B,
    TRAN_GNSS_SERIL_PORT1 = 0x41,
    TRAN_GNSS_SERIL_PORT2 = 0x42,
}JtTransiverDataEnum;
...
typedef enum
{
    ...
    JT_CMD_TRANSIVER_DL = 0x8900,//下行透传
    JT_CMD_TRANSIVER_UP = 0x0900,//上线透传

    JT_CMD_BMS_MSG = 0x0F08, //BMS信息上传
}Jt08CmdType;
...
static Jt08MsgSave s_jt_msg_save = {0, false, };
...
static void protocol_jt_parse_transiver_data(u8* pdata, u16 len);
...
static void protocol_jt_pack_head(u8 *pdata, u16 *idx, u16 len, u16 total, u16 part_num )
{
    ...
    u8 *ptr = NULL;
    u8 i = 0,imei[CONFIG_MAX_CELL_NUM_LEN + 1] = {0};
    ...
    if(ptr == NULL)
    {
        LOG(INFO,"clock(%d) protocol_jt_pack_head can not get imei", util_clock());
    ...
    {
        i = 12 - strlen((const char*)ptr);
    }

    memcpy(imei + i,ptr , 12);

    LOG(INFO,"protocol ime %s",imei);


    pdata[(*idx)++] = MERGEBCD(util_chr(imei[0]), util_chr(imei[1]));
	pdata[(*idx)++] = MERGEBCD(util_chr(imei[2]), util_chr(imei[3]));
	pdata[(*idx)++] = MERGEBCD(util_chr(imei[4]), util_chr(imei[5]));
	pdata[(*idx)++] = MERGEBCD(util_chr(imei[6]), util_chr(imei[7]));
	pdata[(*idx)++] = MERGEBCD(util_chr(imei[8]), util_chr(imei[9]));
...
static void protocol_jt_pack_register_info(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    config_service_get(CFG_JT_OEM_ID, TYPE_STRING, value_string, copy_len);
    ...
    copy_len = 7;
    memset(value_string, 0 , copy_len);
    config_service_get(CFG_JT_DEVICE_ID, TYPE_STRING, value_string, copy_len);
    memcpy(&pdata[(*idx)], value_string, copy_len);
    (*idx) += copy_len;
    ...
    config_service_get(CFG_JT_VEHICLE_NUMBER, TYPE_STRING, value_string, MAX_JT_VEHICLE_NUMBER_LEN);
...
void protocol_jt_pack_up_trans_msg(u8 *pdata, u16 *idx, u16 len ,u8 *updata , u16 uplen)
{
    u8 *send;
    u16 send_len = 0;
    //u16 content_len = 0;
    send = (u8 *)malloc(uplen);
    if (send == NULL)
    {
    ...
    memcpy(&send[send_len],updata,uplen);
    ...
    protocol_jt_pack_id_len(send, &send_len, JT_CMD_TRANSIVER_UP, uplen + 1, 0, false);  // 2bytes
...
void protocol_jt_pack_bms_msg(u8 *pdata, u16 *idx, u16 len ,BmsBatteryInfoStruct *bms)
{
    u8 *send;
    u16 i = 0;
    u16 content_len = 0;
    ...
    if (send == NULL)
    {
        LOG(INFO,"clock(%d) protocol_jt_pack_bms_msg assert(malloc(%d)) failed.", util_clock(), len);
        return;
    }

    protocol_jt_pack_head(send, &i, len, 1, 0);  // 13bytes

    memcpy(&send[i],(u8 *)&bms->code_info[0],16);
    i += 16;
    content_len += 16;

    memcpy(&send[i],(u8 *)&bms->max_vol,120);
    i += 120;
    content_len += 120;

    memset(&send[i],0,64);
    memcpy(&send[i],(u8 *)&bms->bat[0].bat1_vol,8*MAX_BMS_SINGLE_BAT_COUNTS);
    i += 64;
    content_len += 64;

    protocol_jt_pack_id_len(send, &i, JT_CMD_BMS_MSG, content_len, 0, false);  // 2bytes

    *idx = len;
    
    protocol_jt_pack_escape(send, i, pdata, idx);
...
void protocol_jt_pack_regist_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(INFO,"clock(%d) protocol_jt_pack_regist_msg assert(malloc(%d)) failed.", util_clock(), len);
		return;
	}
...
void protocol_jt_pack_auth_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(INFO,"clock(%d) protocol_jt_pack_auth_msg assert(malloc(%d)) failed.", util_clock(), len);
		return;
	}
...
static void protocol_jt_pack_iccid(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    if(GM_SUCCESS != (ret = mm_get_iccid(iccid)))
    {
...
void protocol_jt_pack_logout_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(INFO,"clock(%d) protocol_jt_pack_logout_msg assert(malooc(%d)) failed.", util_clock(), len);
		return;
	}
...
static void protocol_jt_pack_gps_info(GpsDataModeEnum mode, const GPSData *gps, u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    {
        util_utc_sec_to_bcdtime_base2000(get_local_utc_sec(), bcd_tim, zone);
    }
...
static void protocol_jt_pack_additional_imsi(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    if(GM_SUCCESS != (ret = mm_get_imsi(imsi)))
    {
...
void protocol_jt_parse_msg(u8 *pdata, u16 len)
{
    ...
    switch(cmd)
    {
        ...
        case JT_CMD_TEXT:
            ...
            break;
        case JT_CMD_TRANSIVER_DL:
            LOG(DEBUG,"clock(%d) protocol_jt_parse_msg(JT_CMD_TRANSIVER_DL).", util_clock());
            protocol_jt_parse_transiver_data(pdata,len);
            protocol_jt_parse_general_msg(pdata,len);
        default:
...
static void protocol_jt_parse_transiver_data(u8* pdata, u16 len)
{
	...
	cl_log_hex(pdata+data_start,msg_len,"[DATA]clcok(%d) recv 8900:",clock());

    bms_com_write(pdata+data_start+1,msg_len-1);
...
static void protocol_jt_parse_text(u8* pdata, u16 len)
{
    ...
    if (NULL == pOut)
    {
        LOG(WARN,"protocol_jt_parse_text assert(malloc(%d)) failed.", CMD_MAX_LEN + 1);
        return;
    }
   ...
   memset(pOut, 0x00, CMD_MAX_LEN + 1);
    ...
    idx = data_start + 1;
	command_on_receive_data(COMMAND_GPRS, (char *)&pdata[idx], msg_len - 1, (char *)pOut,NULL);
    out_len = strlen((const char *)pOut);
